...
if($action==='list'){
    ...
    $stmt=$conn->prepare('SELECT d.id,d.matter_id,d.original_name,d.mime_type,d.size_bytes,d.sha256,d.status,d.uploaded_by,d.created_at,u.name AS uploaded_by_name FROM documents d LEFT JOIN users u ON u.id=d.uploaded_by INNER JOIN matters m ON m.tenant_id=d.tenant_id AND m.id=d.matter_id WHERE d.tenant_id=? AND d.matter_id=? AND d.deleted_at IS NULL ORDER BY d.created_at DESC');
...
if($action==='upload'){
    ...
    @chmod($destination,0600); $sha=hash_file('sha256',$destination); $uploadedBy=(int)$user['user_id'];
    ...
    if(!$stmt){@unlink($destination);json_response(['status'=>'error','pesan'=>'Layanan dokumen tidak tersedia'],500);}
    $stmt->bind_param('iisssisi',$tenantId,$matterId,$original,$relative,$mime,$size,$sha,$uploadedBy); $stmt->execute(); $docId=$conn->insert_id; $stmt->close();
    $store=$conn->prepare('INSERT INTO document_storage (tenant_id,document_id,storage_key,provider) VALUES (?,?,?,\'local\')');
    if(!$store){@unlink($destination);$conn->query('DELETE FROM documents WHERE id='.(int)$docId.' AND tenant_id='.(int)$tenantId);json_response(['status'=>'error','pesan'=>'Layanan penyimpanan tidak tersedia'],500);}
    $store->bind_param('iis',$tenantId,$docId,$relative); $store->execute(); $store->close();
